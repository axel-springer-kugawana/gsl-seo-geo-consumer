...
* A deprecation notice **MUST** be included in the API documentation months in advance. The notice **MUST** include :
    * the starting date for the deprecation
    * the starting date for the support end of life
    * the impacts if a client do not upgrade to the next version.

**API version deprecation notice example**

> ⚠️ Deprecation notice️
>
> We are deprecating this API version effective **September 30th 2024**.
>
> This API version will see its end of life effective **March 31st 2025**.
>
> What will happen if nothing is done ?
>
> When this API version will reach end of life, any clients or services still using its resources will not work anymore. If you rely on a script, integration, or app that makes requests using this version resources, you will need to update it to use a newer version alternative as soon as possible.

* At runtime, every declared operation in the documentation **MUST** marked as deprecated using the `deprecated` member with `true` as value.
* At runtime, every API response **MUST** include the following headers :
 * a `Deprecation` header with the deprecation date as value.
 * a `Sunset` header with the sunset date as value.
 * a `Link` header with a link to the deprecation notice in the API documentation. Value **MUST** use the following pattern :

    `<{Url}>; rel="deprecation"; type="text/html"`

**Example of an API response containing the required headers**

```
Deprecation: 'Thu, 30 Sep 2021 00:00:00 GMT'
Sunset: 'Thu, 31 Mar 2022 00:00:00 GMT'
Link: '<https://api.my-cool-example.com/doc/index.html#section/Deprecation-notice>; rel="deprecation"; type="text/html"'

```
## Environments

* For internal APIs, environment names **MUST** be included in the subdomain name:

    `https://<capability?>-<api-name>-<env>.<account>.aws.aviv-internal.eu/`

## JSON based payloads

* JSON-based payloads **MUST** conform to these rules:
    * Fields **MUST** follow the `camelCase` convention
    * `null` fields value **SHOULD NOT** be allowed and **SHOULD** be omitted by the client
        * Boolean fields **SHOULD NOT** allow a `null` value and **SHOULD** be omitted by the client
        * Empty arrays **SHOULD NOT** have `null` as value, instead the value **SHOULD** be an empty array (`[]`) or **SHOULD** be omitted
        * String fields **SHOULD NOT** allow `null` values and **SHOULD** be omitted by the client
    * Array fields **SHOULD** be plural (e.g. `classifieds: [...]`, `agencies: [...]`)

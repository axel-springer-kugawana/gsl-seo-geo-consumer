...
locals {
  ...
  container_environment = [
    ...
    { name = "GEO_LEGACY_MAPPING_BUCKET_NAME", value = var.geo_legacy_mapping_bucket.name },
...
resource "aws_ecs_task_definition" "geo_legacy_mapping_load" {
  family                   = "${var.application}-${var.environment}-geo-legacy-mapping-load"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = var.task_cpu
  memory                   = var.task_memory
  ...
  container_definitions = jsonencode([
    {
      name      = "geo-legacy-mapping-load"
      image     = "${aws_ecr_repository.geo_bulk_load.repository_url}:${local.selected_image_tag}"
      essential = true
      environment = concat(local.container_environment, [
        { name = "GEO_BULK_LOAD_TASK", value = "geo-legacy-mapping-load" },
      ])

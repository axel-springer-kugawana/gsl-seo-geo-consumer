...
router.post('/:lambda/:handler/', async (req: Request, res: Response) => {
  ...
  if (!lambdaEntry) {
    return res
      ...
      .json({
        error:
          'lambda entrypoint not found. An entrypoint should be the path of the lambda you want to invoke. Please make sure that your entrypoint is defined in `lambda-entrypoints.config.ts` file'
      })
      .end();
  }
  ...
  if (!lambdaFunctionHandler) {
    return res
      ...
      .json({
        error:
          'lambda handler not found. Please make sure that your lambda handler function is exported from your lambda entrypoint.'
      })
      .end();
  }
...
app.listen(port, async () => {
  console.log(
    `[Local λ debugger]: Local lambda invoke debug server is running at http://localhost:${port}`
  );
  console.log(
    `[Local λ debugger]: Discovered ${lambdasEntrypoints.length} lambdas entrypoints`
  );
  ...
  for (let index = 0; index < lambdasEntrypoints.length; index++) {
    ...
    const exportedFunctions = await getExportedFunctionsFromModule(
      element.entryPoint
    );

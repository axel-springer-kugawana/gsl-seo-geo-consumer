...
export class EventEnvelope<EventType extends string, TData> extends  CloudEvent<TData> {
    ...
    static createEventNotificationEnvelope<EventType extends string, TData>(event: EventNotificationEnvelopeProperties<EventType, TData>) {
        event.eventcategory = 'NotificationEvent';
        return new EventEnvelope(event);
    }

    static createIntegrationEventEnvelope<EventType extends string, TData>(event: IntegrationEventEnvelopeProperties<EventType, TData>) {

  ...
  await pgClient.query(`DROP INDEX IF EXISTS ${PG_SCHEMA}.idx_geofeature_streets_by_municipality;`);
    ...
    await duckDBConnection.run(`
      ...
      SELECT 
        ...
        (CASE WHEN TYPE_LEVEL = 600 THEN ID ELSE (AD06->>0)::VARCHAR END) AS provinceId,
        (CASE WHEN TYPE_LEVEL = 800 THEN ID ELSE (AD08->>0)::VARCHAR END) AS municipalityId,
      ...
      await pgClient.query(`
        CREATE OR REPLACE VIEW ${PG_SCHEMA}.v_geo_full
         ...
         SELECT geo.avivgeoid,
            ...
            borough.code AS boroughcode,
            borough.fictive AS boroughfictive,
            borough.level AS boroughlevel,
            borough.names AS boroughnames,
            neighborhood.code AS neighborhoodcode,
            neighborhood.fictive AS neighborhoodfictive,
            neighborhood.level AS neighborhoodlevel,
            neighborhood.names AS neighborhoodnames,
            microneighborhood.code AS microneighborhoodcode,
            microneighborhood.fictive AS microneighborhoodfictive,
            microneighborhood.level AS microneighborhoodlevel,
            microneighborhood.names AS microneighborhoodnames,
            geo.names,
